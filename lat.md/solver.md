...
Precomputed pairwise compatibility scores between all pokemon. Type: `Map<string, Map<string, number | null>>` (exported as `AdjacencyMap` from `src/solver.ts`).

Loaded from the `adjacency` table in `public/pokehousing.sqlite` by HomeView on mount and passed as the optional `adjacencyMap` argument to `solve`. Values are shared-favorites count plus habitat adjustments (see [[lat.md/solver#Solver#Clustering Pre-assignment#Habitat Compatibility]]); `null` signals structural incompatibility (opposite habitat axis) — the pair is excluded from clustering and matching entirely.
