...
The recommended items table uses the CSS class `recommended-items-table`, which rotates header text diagonally (~50°) via `transform: rotate(-50deg)` on a `<span>` wrapper inside each `<BTh>`, keeping column widths determined by body content rather than header text.
