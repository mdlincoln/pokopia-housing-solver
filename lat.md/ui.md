...
The recommended items table uses the CSS class `recommended-items-table`, which rotates every header's text vertically (90°) via `writing-mode: vertical-rl` + `transform: rotate(180deg)` on the `<span>` wrapper inside each `<th>`. Boolean columns (tag and favorite columns) carry the CSS class `bool-col` via `field.class`, which Bootstrap Vue Next applies to both `<th>` and `<td>` for those fields. `bool-col` enforces 40px width, centers content, and adds a left border that creates a visible vertical track from header into body rows. A `#cell()` catch-all slot renders a large bold green `✓` (`.bool-check`) when the cell value is truthy; false cells are empty and neutral. Sort arrows for sortable headers are painted at the bottom-center of the `<th>` (via a `background-position: center bottom` override) so they sit below the rotated text instead of overlapping it.

The recommended items inside each house card use a `BTable` with one row per item (no rowspan grouping). Columns are: item name with flavor text tooltip (`data-testid="item-name"`), picture thumbnail (`item-thumbnail` CSS, 32×32), add-to-cart button (`data-testid="add-to-cart"`), craftability text (`data-testid="item-craftability"` — "Craftable - {category}" for craftable items, "Buy" otherwise), up to three boolean tag columns (`col_toy`, `col_relaxation`, `col_decoration`), and one column per distinct unfulfilled favorite of pokemon assigned to the house. Tag columns are omitted once that tag is fulfilled by any cart item. Favorite columns are ordered by frequency (most-shared first, ties sorted alphabetically). The entire recommendations `<details>` panel is hidden once every pokemon favorite for the house has been fulfilled by cart items — the watcher clears `activeTableItems` rather than re-listing already-covered items with no ranking columns. The table uses `sticky-header` and is sorted by BTable's built-in sort; the default sort is the first favorite column descending so items covering the most-common favorite appear at the top. Every header wrapper is a `<span>` so the vertical-rotation CSS applies uniformly. The `#head()` slot adds fulfillment styling (`text-success`/`text-danger`) to tag and favorite headers.
...
Items are grouped by house (`data-testid="cart-house-group"`), with a heading showing the house ID. A one-line legend above the groups explains the stamp. Each cart item shows a picture thumbnail, item name (with flavor text tooltip), craft/buy and category badges (see [[ui#House#Item Metadata Display]]), and a `×` remove button. Below the badges, one **progress stamp** label (styled pill-shaped `<label>` element with hidden `<input type="checkbox">`) communicates crafted state: the **🔨 Crafted** stamp (`progress-step--craft`, amber palette, `data-testid="progress-checkbox-cart-item"`) toggles `.checked-off` and strikethrough on the item name and removes the item's ingredients from aggregated totals. Below all house groups, aggregated ingredient totals (excluding crafted items) are shown as a `BListGroup`. Items with no recipe show "(no recipe)" in muted text.
